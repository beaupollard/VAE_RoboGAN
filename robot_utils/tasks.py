...
class ForwardSpeedTask(ABC):
  def __init__(self, time_step=1.0/240, discount_factor=0.99, interval=16,
               horizon=16, episode_len=512, noise_seed=0, force_std=0.0,
               torque_std=0.0):
...
class ForwardSpeedTask0(ABC):
  def __init__(self, time_step=1.0/240, discount_factor=0.99, interval=16,
    ...
    self.objective_fn = rd.DotProductObjective()
    self.objective_fn.base_dir_weight = np.array([-2.0, -2.0, 0.0])
    self.objective_fn.base_up_weight = np.array([0.0, 2.0, 0.0])
    self.objective_fn.base_vel_weight = np.array([10.0, 10.0, 0.0])

    # Maximum reasonable result (designs achieving higher results are rejected)
    self.result_bound = 15.0
...
class ForwardSpeedTask2(ABC):
  def __init__(self, time_step=1.0/240, discount_factor=0.99, interval=16,
               horizon=32, episode_len=512, noise_seed=0, force_std=0.0,
               torque_std=0.0):
    self.time_step = time_step
    self.discount_factor = discount_factor
    self.interval = interval
    ...
    self.objective_fn.base_dir_weight = np.array([-2.0, -2.0, 0.0])
    self.objective_fn.base_up_weight = np.array([0.0, 2.0, 0.0])
    self.objective_fn.base_vel_weight = np.array([10.0, 20.0, 0.0])
...
class ForwardSpeedTask3(ABC):
  def __init__(self, time_step=1.0/240, discount_factor=0.99, interval=16,
    ...
    self.objective_fn = rd.DotProductObjective()
    self.objective_fn.base_dir_weight = np.array([-2.0, -1.0, 0.0])
    self.objective_fn.base_up_weight = np.array([0.0, 2.0, 0.0])
    self.objective_fn.base_vel_weight = np.array([10.0, 5.0, 0.0])
...
class ForwardSpeedTask4(ABC):
  def __init__(self, time_step=1.0/240, discount_factor=0.99, interval=16,
    ...
    self.objective_fn.base_dir_weight = np.array([-2.0, -1.0, 0.0])
    self.objective_fn.base_up_weight = np.array([0.0, 2.0, 0.0])
    self.objective_fn.base_vel_weight = np.array([10.0, 1.0, 0.0])
...
class ForwardSpeedTask5(ABC):
  def __init__(self, time_step=1.0/240, discount_factor=0.99, interval=16,
    ...
    self.objective_fn.base_dir_weight = np.array([-2.0, -1.0, 0.0])
    self.objective_fn.base_up_weight = np.array([0.0, 2.0, 0.0])
    self.objective_fn.base_vel_weight = np.array([10.0, 8.0, 0.0])
...
class ForwardSpeedTask6(ABC):
  def __init__(self, time_step=1.0/240, discount_factor=0.99, interval=16,
    ...
    self.objective_fn = rd.DotProductObjective()
    self.objective_fn.base_dir_weight = np.array([-2.0, -2.0, 0.0])
    self.objective_fn.base_up_weight = np.array([0.0, 5.0, 0.0])
    # self.objective_fn.base_up_weight = np.array([0.0, 0.0, 0.0])
    self.objective_fn.base_vel_weight = np.array([25.0, 15.0, 0.0]) # This is the original one
    # self.objective_fn.base_vel_weight = np.array([5.0, 10.0, 0.0])
    # self.objective_fn.base_vel_weight = np.array([10.0, 0.0, 0.0])
  ...
  def add_noise(self, sim, time_step_idx):
    ...
    sim.add_link_force_torque(0, 0,
                                noise_rng.normal(0.0, self.force_std, size=3),
                                noise_rng.normal(0.0, self.torque_std, size=3))
    # for i in range(sim.get_robot_dof_count(0)):
    #   sim.add_link_force_torque(0, i,
    #                             np.ones(3)*0.1,
    #                             np.ones(3)*0.1)
...
class SteppedTerrainTask(ForwardSpeedTask0):
  """
...
class CustomizedSteppedTerrainTask1(ForwardSpeedTask1):
  """
  Task where the objective is to move forward as quickly as possible on stepped
  terrain.
  Wide stairs
...
class CustomizedSteppedTerrainTask2(ForwardSpeedTask2):
  """
  Task where the objective is to move forward as quickly as possible on stepped
  terrain.
  Tall stairs
  ...
  def __init__(self, x_min=-20.0, x_max=5.0, seed=0, **kwargs):
    super().__init__(**kwargs)
    self.seed = seed
...
class CustomizedSteppedTerrainTask3(ForwardSpeedTask1):
  """
  Task where the objective is to move forward as quickly as possible on stepped
  terrain.
  Wide stairs
  ...
  def __init__(self, x_min=-20.0, x_max=20.0, seed=0, **kwargs):
    ...
    rng = np.random.RandomState(self.seed)
    edge_x = np.arange(0.0, x_max, 0.25)
    # edge_x += rng.normal(0.0, 0.1, size=edge_x.shape)
    platform_x_min = np.concatenate(([x_min], edge_x))
    platform_x_max = np.concatenate((edge_x, [x_max]))
    platform_x = 0.5 * (platform_x_min + platform_x_max)
...
class CustomizedWallTerrainTask1(ForwardSpeedTask6):
  """
  ...
  def __init__(self, seed=0, **kwargs):
    super().__init__(horizon=64, **kwargs) #was 128
    self.seed = seed

    self.floor = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [20.0, 1.0, 10.0])
    self.wall = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [0.05, 0.25, 1.25])
...
class CustomizedWallTerrainTask2(ForwardSpeedTask4):
  """
  Task where the objective is to move forward as quickly as possible around a
  series of walls.
  """
  ...
  def __init__(self, seed=0, **kwargs):
    ...
    self.floor = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [20.0, 1.0, 10.0])
    self.wall = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [0.05, 0.5, 0.5])
    self.side_wall = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [20, 0.5, 0.05])
...
class CustomizedBiModalTerrainTask1(ForwardSpeedTask3):
  """
  Task where the objective is to move forward as quickly as possible around a
  series of walls.
  """
  ...
  def __init__(self, x_min=-20.0, x_max=20.0, seed=0, **kwargs):
    ...
    self.wall1 = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [0.1, 0.05, 0.1])
    self.wall2 = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [0.1, 0.15, 0.1])
  def add_terrain(self, sim):
    rng = np.random.RandomState(self.seed)
    for i, (x, platform) in enumerate(zip(self.platform_x, self.platforms)):
      if i > 0:
        ...
        if i % 2 == 0:
          mult = -1
        sim.add_prop(self.wall1,
                   [0.2 * i + 0.3, 0.0, 0 - 0.1*mult],
                   rd.Quaterniond(1.0, 0.0, 0.0, 0.0))
        sim.add_prop(self.wall2,
                   [0.2 * i + 0.3, 0.0, 0 + 0.1*mult],
                   rd.Quaterniond(1.0, 0.0, 0.0, 0.0))
        
        sim.add_prop(self.wall1,
                   [0.2 * i + 0.3, 0.0, -0.4 - 0.1*mult],
                   rd.Quaterniond(1.0, 0.0, 0.0, 0.0))
        sim.add_prop(self.wall2,
                   [0.2 * i + 0.3, 0.0, -0.4 + 0.1*mult],
                   rd.Quaterniond(1.0, 0.0, 0.0, 0.0))
...
class CustomizedBiModalTerrainTask2(ForwardSpeedTask7):
  """
  Task where the objective is to move forward as quickly as possible over
  terrain with several large gaps.
  """

  def __init__(self, x_min=-20.0, x_max=50.0, seed=0, **kwargs):
    super().__init__(**kwargs)
    self.seed = seed
...
class CustomizedBiModalTerrainTask3(ForwardSpeedTask5):
  """
  Task where the objective is to move forward as quickly as possible around a
  series of walls.
  """
...
class CustomizedRidgesTerrainTask1(ForwardSpeedTask):
  """
  Task where the objective is to move forward as quickly as possible over ridged
  terrain.
  """
  ...
  def __init__(self, seed=0, **kwargs):
    ...
    self.floor = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [20.0, 1.0, 10.0])
    self.bump = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [0.2, 0.2, 10.0])
...
class CustomizedRidgesTerrainTask2(ForwardSpeedTask):
  """
  Task where the objective is to move forward as quickly as possible over ridged
  terrain.
  """
  ...
  def __init__(self, seed=0, **kwargs):
    ...
    self.floor = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [20.0, 1.0, 10.0])
    self.bump = rd.Prop(rd.PropShape.BOX, 0.0, 0.5, [0.4, 0.2, 10.0])
...
# class CustomizedGapTerrainTask1(ForwardSpeedTask):
#   """
#   Task where the objective is to move forward as quickly as possible over
#   terrain with several large gaps.
#   """
...
class CustomizedGapTerrainTask2(ForwardSpeedTask):
  """
  Task where the objective is to move forward as quickly as possible over ridged
  terrain.
  """
  ...
  def add_terrain(self, sim):
    ...
    for i in range(20):
      if i == 0:
        sim.add_prop(self.bump,
                   [rng.normal(0.5, 0.05) + i, -0.15, 0.0],
                   rd.Quaterniond(1.0, 0.0, 0.0, 0.0))
        continue
      if i == 1:
        sim.add_prop(self.bump,
                   [rng.normal(0.5, 0.05) + i, -0.13, 0.0],
                   rd.Quaterniond(1.0, 0.0, 0.0, 0.0))
        continue
      sim.add_prop(self.bump,
                   [rng.normal(0.5, 0.05) + i, -0.2 + rng.uniform(0.06, min(0.20, 0.09 * (i + 1))), 0.0],
...
class CustomizedTerrainTask1(ForwardSpeedTask):
  """
  Task where the objective is to move forward as quickly as possible over ridged
  terrain.
  """
  ...
  def add_terrain(self, sim):
    ...
    cnt = 0
    for i, (x, platform) in enumerate(zip(self.platform_x, self.platforms)):
      rnd = rng.normal(0.0, 0.01 * i)
      sim.add_prop(platform, [x, -1.0 + rnd, 0.0],
                   rd.Quaterniond(1.0, 0.0, 0.0, 0.0))
      if cnt == 2:
        break
