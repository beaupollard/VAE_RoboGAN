...
from .uniform_reward_net_total_onehot_testflat_softmax import RewardNet_onehot_softmax

vocab_size = VOCAB_SIZE
class JTNNVAE_RW_SOFTMAX(nn.Module):

    def __init__(self, hidden_size, latent_size, depthT, encoding, pred_prop):
        super(JTNNVAE_RW_SOFTMAX, self).__init__()
        ...
        if pred_prop:
            self.predictor = RewardNet_onehot_softmax(input_length = 28, n_hidden_layers=1, hidden_layer_size = 128,
    ...
    def dis_pred_loss(self, z_vecs, W_mean, loc_batch):
        ...
        X_env[:,0] += 1
        dis = self.predictor(z_mean, X_env)
