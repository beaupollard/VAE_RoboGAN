    ...
    def tensorize(tree_batch):
        ...
        for tree in tree_batch:
            ...
            for node in tree.nodes:
                if len(node.neighbors) == 1 and node.wid // N_JOINT != 1:
                    tree_leaf.append(node.idx)
            leaf.append(tree_leaf)

        return JTNNEncoder.tensorize_nodes(node_batch, scope, leaf)
    
    @staticmethod
    def tensorize_nodes(node_batch, scope, leaf):
...
class GraphGRU(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, depth):
        ...
        self.drop_out = LockedDropout(0.2)
