...
reward_model = JTNNVAE_DIS(450, 28, 20, "sum", True).cuda()
...
# ckpt = torch.load('vae_train_rw_dis_model_4k/best_model.pth')
...
def estimate_reward(X,k_one_hot):
    ...
    out = reward_model(batch, None, beta, None, None)
    # Y.append(out)
...
def ga_objective(x,k,reward_model):
    ...
    return -reward_model(batch, None, beta, 0, 0)[0].item(), -reward_model(batch, None, beta, 0, 0)[0].item(), (20 - num_modules)/6 
...
def data_creation(gen_design,k_list):
    ...
    for i in range(batch_size):
        k = k_list[i]
        best, score, _, _ = ga_one_iter(ga_objective, num_var, varmin, varmax, 10, 
...
def generate_designs(k, GAN_ckpt, epoch, iter, vae_file = "sum_ls28_pred20/model.iter-400000", batch = 8):
    ...
    for i in tqdm(range(out.shape[0])):
        ...
        if result_cache_key in result_cache:
            ...
            result = get_distance(robot, task)
...
if __name__ == "__main__":
    ...
    if train:
        iteration = 50
        ckpt_path = "GAN_ckpt_v34_newVAERW_updateRW_FLAT_only"
    ...
    # # ----------
    if generate:
        generate_designs(0, 'GAN_ckpt_v34_newVAERW_updateRW_FLAT_only', 9, 50, vae_file = "sum_ls28_pred20/model.iter-400000",batch = 10)
    #     # for i in range(9):
    #     #     generate_designs(i, 'GAN_ckpt_v34_newVAERW_updateRW_FLAT_only', 10, vae_file = "sum_ls28_pred20/model.iter-400000",batch = 10)
